...
The primary UI imports and reads PDF/EPUB files locally. React calls `LocalLibraryService` and `ReaderService`; repositories own IndexedDB, `OpfsBookBinaryStorage` owns browser file handles, and lazy PDF.js/Foliate adapters own renderer state. Core code does not import the legacy API client or wait for Spring Boot. A generated Workbox service worker precaches the production application shell and reader chunks, while install/update/connectivity/navigation events remain behind `platform/web`.
...
Reader contracts, locator examples, controls, security boundaries, and limitations are documented in [reader engines](../docs/architecture/reader-engines.md). The EPUB choice and current renderer comparison are in [ADR-002](../docs/architecture/adr-002-epub-renderer.md).
